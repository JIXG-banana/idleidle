    ...
    <div className="flex flex-col gap-6 p-4">
      ...
      <div className="p-4 bg-gray-100 rounded-xl text-center">
        ...
        <div className="w-full bg-gray-300 h-2 rounded-full mt-2 overflow-hidden">
          <div 
            className={`${isCapacityReached ? "bg-red-600 animate-pulse" : "bg-red-500"} h-full transition-all duration-500`} 
            style={{ width: `${Math.min(100, money.log10() / 60 * 100)}%` }}
          />
        </div>
        <p className="text-[10px] text-gray-400 mt-2 italic">
          {isCapacityReached 
            ? <button onClick={onResetCapacity} className="text-red-600 font-bold underline">⚡ {t("capacity.reset_btn") || "世界をリセットして 1 CP を獲得"}</button>
            : `${t("capacity.hint") || "1.00e60 Gold 到達でリセット可能"}`}
        </p>
      </div>
